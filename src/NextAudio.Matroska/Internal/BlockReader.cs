...
internal static class BlockReader
{
    ...
    {
        var vInt = await VIntReader.ReadVIntAsync(stream, buffer, position, logger);
        ...
        position = await AudioStreamUtils.SeekAsync(stream, 2, SeekOrigin.Current, position);

        position += await AudioStreamUtils.ReadFullyAudioStreamAsync(stream, buffer[vInt.Length..(vInt.Length + 1)]);
        ...
        if (lacingType != MatroskaBlockLacingType.No)
        {
            position += await AudioStreamUtils.ReadFullyAudioStreamAsync(stream, buffer[(vInt.Length + 1)..(vInt.Length + 2)]);
        ...
        switch (lacingType)
        {
            ...
            case MatroskaBlockLacingType.Xiph:
                position = await ParseXiphLacingAsync(stream, buffer[(vInt.Length + 2)..], position, blockElement, frameSizes);
                break;
            case MatroskaBlockLacingType.FixedSize:
                ParseFixedSizeLacing(position, blockElement, frameSizes);
                break;
            case MatroskaBlockLacingType.Ebml:
                position = await ParseEbmlLacingAsync(stream, buffer[(vInt.Length + 2)..], position, blockElement, frameSizes, logger);
                break;
        }
    ...
    private static async ValueTask<long> ParseXiphLacingAsync(AudioStream stream, Memory<byte> buffer, long position, MatroskaElement blockElement, Memory<int> frameSizes)
    {
        ...
        for (var i = 0; i < frameSizes.Length - 1; i++)
        {
            ...
            do
            {
                position += await AudioStreamUtils.ReadFullyAudioStreamAsync(stream, buffer[bufferIndex..(bufferIndex + 1)]);
    ...
    private static async ValueTask<long> ParseEbmlLacingAsync(AudioStream stream, Memory<byte> buffer, long position, MatroskaElement blockElement, Memory<int> frameSizes, ILogger logger)
    {
        ...
        for (var i = 1; i < frameSizes.Length - 1; i++)
        {
            vInt = await VIntReader.ReadVIntAsync(stream, buffer[totalVIntLength..], position, logger);
